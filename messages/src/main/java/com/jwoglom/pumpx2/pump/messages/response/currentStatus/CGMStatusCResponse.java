...
/**
 * Returns CGM (Continuous Glucose Monitor) status.
 *
 * This is the CGMStatusB layout (see {@link CgmStatusV2Response}) with the sensor warmup
 * durations appended.
 *
 ...
 *   offset 20 (2 bytes): sensorWarmupDuration     - uint16, seconds
 *   offset 22 (2 bytes): remainingWarmupDuration  - uint16, seconds
 */
@MessageProps(
    opCode=-51,
    size=24,
    ...
    request=CGMStatusCRequest.class
)
public class CGMStatusCResponse extends Message {
    ...
    private int sensorWarmupDurationSeconds;
    private int remainingWarmupDurationSeconds;

    public CGMStatusCResponse() {
    }
    ...
    public CGMStatusCResponse(
            ...
            int cgmSensorTypeId,
            boolean gracePeriod,
            int sensorWarmupDurationSeconds,
            int remainingWarmupDurationSeconds) {
        this.cargo = buildCargo(
                ...
                cgmSensorTypeId,
                gracePeriod,
                sensorWarmupDurationSeconds,
                remainingWarmupDurationSeconds);
        parse(this.cargo);
    }
    ...
    public void parse(byte[] raw) {
        ...
        this.sensorWarmupDurationSeconds = Bytes.readShort(raw, 20);
        this.remainingWarmupDurationSeconds = Bytes.readShort(raw, 22);
    }
    ...
    public static byte[] buildCargo(
        ...
        return Bytes.combine(
                ...
                Bytes.firstTwoBytesLittleEndian(sensorWarmupDurationSeconds),
                Bytes.firstTwoBytesLittleEndian(remainingWarmupDurationSeconds));
    }
    ...
    /**
     * @return the total warmup duration for the current sensor, in seconds
     */
    public int getSensorWarmupDurationSeconds() {
        return sensorWarmupDurationSeconds;
    }

    /**
     * @return the warmup time remaining for the current sensor, in seconds
     */
    public int getRemainingWarmupDurationSeconds() {
        return remainingWarmupDurationSeconds;
    }
}
