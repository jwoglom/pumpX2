...
/**
 * CGM Fall Rate Alert (FRA) settings change history log entry.
 *
 * Fields parsed from the decompiled CgmGlucoseAlertSettingsModel:
 *   alertLevel   (raw[10-11]): uint16 - the fall rate threshold value (mg/dL/min)
...
@HistoryLogProps(
    opCode = 168,
    displayName = "CGM FRA Settings",
    internalName = "LID_CGM_FRA_SETTINGS"
)
public class CgmFraSettingsHistoryLog extends HistoryLog {
    ...
    public CgmFraSettingsHistoryLog() {}
    public CgmFraSettingsHistoryLog(long pumpTimeSec, long sequenceNum, int alertLevel, int repeatDuration, boolean isEnabled, int modifiedField) {
    ...
    public CgmFraSettingsHistoryLog(int alertLevel, int repeatDuration, boolean isEnabled, int modifiedField) {
    ...
    /**
     * @return the fall rate alert threshold value (mg/dL/min)
     */
    ...
    /**
     * @return whether the fall rate alert is enabled
     */
