...
/**
 * IOB snapshot written when a BLE, pump-UI or Control-IQ bolus is initiated, before the bolus
 * is delivered; {@link IobSnapshotPostBolusHistoryLog} is its counterpart at completion.
 *
 ...
 * no name for opcode 206, so this class name is provisional.
 * See https://github.com/jwoglom/pumpX2/issues/143.
 */
@HistoryLogProps(
    opCode = 206,
    displayName = "IOB Snapshot Before Bolus"
)
public class IobSnapshotPreBolusHistoryLog extends HistoryLog {
    ...
    public IobSnapshotPreBolusHistoryLog() {}

    public IobSnapshotPreBolusHistoryLog(long pumpTimeSec, long sequenceNum, int bolusId, int unknownU8At12, int selectedIob, float mudaliarIob, float swan6hrIob, float unknownFloatAt22) {
        this(pumpTimeSec, sequenceNum, bolusId, unknownU8At12, selectedIob, mudaliarIob, swan6hrIob, unknownFloatAt22, 0);
    }

    public IobSnapshotPreBolusHistoryLog(long pumpTimeSec, long sequenceNum, int bolusId, int unknownU8At12, int selectedIob, float mudaliarIob, float swan6hrIob, float unknownFloatAt22, int headerHighNibble) {
    ...
    public static byte[] buildCargo(long pumpTimeSec, long sequenceNum, int bolusId, int unknownU8At12, int selectedIob, float mudaliarIob, float swan6hrIob, float unknownFloatAt22, int headerHighNibble) {
        return HistoryLog.fillCargo(Bytes.combine(
            HistoryLog.typeIdBytes(206, headerHighNibble),
     ...
     * insulin duration. When {@link #getSelectedIob()} is 0 this is the IOB
     * shown on the pump.
     */
     ...
     * When {@link #getSelectedIob()} is 1 this is the IOB shown
     * on the pump.
     */
