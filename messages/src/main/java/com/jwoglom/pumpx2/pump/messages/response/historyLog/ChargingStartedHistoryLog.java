...
/**
 * Battery snapshot written when charging starts, on both the Mobi and the t:slim X2. Every
 * {@link CurrentBatteryV2Response} chargingStatus 0 to 1 transition seen in the maintainer's and a
 * tester's Mobi captures (9 of 9) has one within seconds, and every record matches such a
 * transition. On the t:slim X2 it usually follows a {@link UsbConnectedHistoryLog} by a few
 * seconds.
 *
 * The class name is provisional: opcode 34 is not in Tandem's cloud event schema or the Mobi
 * app's history log list. {@link ChargingStoppedHistoryLog} has the same layout. Fields not named
...
@HistoryLogProps(
    opCode = 34,
    displayName = "Charging Started"
)
public class ChargingStartedHistoryLog extends HistoryLog {
    ...
    public ChargingStartedHistoryLog() {}
    public ChargingStartedHistoryLog(long pumpTimeSec, long sequenceNum, int currentBatteryAbc, int unknown12, int unknown14, int currentBatteryIbc, int unknown18, int lipoMv, int unknown22, int unknown24) {
    ...
    /**
     * 0 on the t:slim X2 and on Mobi firmware 7.9.0.2. On an older-firmware Mobi it was 0, 3 or 7.
     *
